...
void show_char(u16 x,u16 y,char hstr,char lstr)
{
	...
	//write
	SD_ReadSingleBlock(((94*(hstr-0xa0-1)+(lstr-0xa0-1))*32)/512+binadd,string);//str/512+3686408,string);
	for(pos=0;pos<16;pos++)
	{
		temp=string[pos+num+((94*(hstr-0xa0-1)+(lstr-0xa0-1))*32)%512];		 
		for(t=7;t>=0;t--)
		{                 
			if(temp&0x01)LCD_DrawPoint(x+pos,y-t);     
			temp>>=1; 
